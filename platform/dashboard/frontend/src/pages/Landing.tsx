...
const pipelineStages = [
  {
    ...
    icon: Image,
    title: 'Scrape',
    description: 'Collect creative ads from multiple platforms automatically',
...
const dashboardPages = [
  {
    ...
    features: ['Assets scraped counter', 'Pipeline throughput chart', 'Industry distribution pie', 'Queue status & recent jobs'],
  ...
  },
  {
    ...
    features: ['Bulk selection & actions', 'Filter by status/type', 'Retry failed jobs', 'Real-time status updates'],
  ...
  },
  {
    ...
    features: ['Grid & list view modes', 'Filter by industry/source', 'Copy prompts to clipboard', 'Reprocess assets'],
  ...
  },
  {
    ...
    features: ['Trigger scraping jobs', 'Source selection', 'Success rate tracking', 'Rate limit monitoring'],
  ...
  },
  {
    ...
    features: ['Multi-metric time series', 'Quality distribution', 'CTA type breakdown', 'Industry trends'],
  ...
  },
  {
    ...
    features: ['Filter by level/service', 'Full-text search', 'Color-coded entries', 'Real-time refresh'],
  ...
  return (
    <div className="min-h-screen bg-surface-950 overflow-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -bottom-1/2 -right-1/2 w-full h-full bg-gradient-radial from-purple-500/10 via-transparent to-transparent animate-pulse-slow" style={{ animationDelay: '1.5s' }} />
      ...
      <section className="relative z-10 flex flex-col items-center justify-center px-6 pt-12 pb-16 lg:pt-16 lg:pb-20">
        ...
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-brand-500/10 border border-brand-500/20 mb-8 animate-fade-in">
          ...
          <span className="text-sm font-medium text-brand-400">AI-Powered Creative Intelligence</span>
        </div>
        ...
        {/* Subtitle */}
        <p className="mt-6 text-lg text-surface-400 text-center max-w-2xl animate-slide-up" style={{ animationDelay: '0.1s' }}>
          Scrape, analyze, and reverse-engineer creative advertisements into 
          detailed prompts for AI image generation.
        </p>

        {/* CTA buttons */}
        <div className="flex flex-col sm:flex-row items-center gap-4 mt-8 animate-slide-up" style={{ animationDelay: '0.2s' }}>
          <Link
        ...
        {/* Stats */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-12 mt-16 animate-fade-in" style={{ animationDelay: '0.3s' }}>
          {stats.map((stat) => (
      ...
      {/* Pipeline Section */}
      <section id="pipeline" className="relative z-10 px-6 lg:px-12 py-20 border-t border-surface-800/50">
            ...
            {pipelineStages.map((stage, index) => (
              <div key={stage.title} className="relative group">
                ...
                <div className="relative p-6 rounded-2xl bg-surface-900/50 border border-surface-800 hover:border-surface-700 transition-all duration-300 h-full">
                  ...
                  <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${stage.color} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}>
            ...
            {dashboardPages.map((page) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className={`w-12 h-12 rounded-xl ${page.iconBg} flex items-center justify-center flex-shrink-0`}>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-surface-400">System status, appearance, service health & notifications</p>
                </div>
              </div>
          ...
          <div className="mt-12 p-6 rounded-2xl bg-surface-900/50 border border-surface-800">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {refreshIntervals.map((item) => (
                <div key={item.data} className="flex items-center justify-between p-3 rounded-lg bg-surface-800/50">
...
│ │Pipeline │                                                      │
│ ├─────────┤  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌─────────┐│
│ │⚗️ Templ. │  │`}<span className="text-blue-400">Assets</span>{`   │ │`}<span className="text-purple-400">Features</span>{` │ │`}<span className="text-emerald-400">Prompts</span>{`  │ │`}<span className="text-rose-400">Errors</span>{`  ││
│ │  Mock   │  │ `}<span className="text-white">1,234</span>{`    │ │ `}<span className="text-white">1,100</span>{`    │ │ `}<span className="text-white">987</span>{`      │ │ `}<span className="text-white">0.5%</span>{`   ││
│ │  data   │  │ `}<span className="text-emerald-400">▲ 12.5%</span>{`  │ │ `}<span className="text-emerald-400">▲ 8.3%</span>{`   │ │ `}<span className="text-emerald-400">▲ 15.2%</span>{`  │ │ `}<span className="text-emerald-400">▼ 2.1%</span>{` ││
│ ├─────────┤  └──────────┘ └──────────┘ └──────────┘ └─────────┘│
│ │📊 Dash  │                                                      │
│ │📋 Jobs  │  ┌─────────────────────────┐ ┌────────────────────┐ │
│ │🖼️ Assets│  │  `}<span className="text-cyan-400">Pipeline Throughput</span>{`     │ │  `}<span className="text-amber-400">Industry Dist.</span>{`   │ │
│ │📡 Scrape│  │  `}<span className="text-blue-400">▓▓▓▓▓▓</span>{`                │ │    `}<span className="text-blue-400">●</span>{` Finance     │ │
│ │📈 Analyt│  │  `}<span className="text-blue-400">▓▓▓▓▓▓▓▓▓▓</span>{`            │ │    `}<span className="text-purple-400">●</span>{` E-commerce  │ │
│ │📝 Logs  │  │  `}<span className="text-blue-400">▓▓▓▓▓▓▓▓</span>{`              │ │    `}<span className="text-emerald-400">●</span>{` SaaS        │ │
│ │⚙️ Settin│  └─────────────────────────┘ └────────────────────┘ │
│ ├─────────┤                                                      │
│ │🏠 Home  │  ┌─────────────────────────┐ ┌────────────────────┐ │
│ ├─────────┤  │  `}<span className="text-surface-300">Queue Status</span>{`          │ │  `}<span className="text-surface-300">Recent Jobs</span>{`      │ │
│ │System   │  │  Pending:     `}<span className="text-amber-400">15</span>{`       │ │  scrape    `}<span className="text-emerald-400">●done</span>{` │ │
│ │Status:  │  │  In Progress: `}<span className="text-blue-400">3</span>{`        │ │  extract   `}<span className="text-blue-400">●run</span>{`  │ │
│ │`}<span className="text-emerald-400">✓ Online</span>{` │  │  Completed:   `}<span className="text-emerald-400">542</span>{`      │ │  generate  `}<span className="text-amber-400">●wait</span>{` │ │
│ └─────────┘  └─────────────────────────┘ └────────────────────┘ │
└─────────────────────────────────────────────────────────────────┘`}
              </pre>
            </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-surface-400 mb-4">
                  The dashboard runs with <strong className="text-surface-300">mock data</strong> for demonstration purposes. 
                  In production, you would connect to real GCP services (Firestore, Cloud Run, Pub/Sub) for live data.
                </p>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2 text-surface-500">
              ...
              <span className="text-sm">Tasmem Creative Ads Platform</span>
            </div>
