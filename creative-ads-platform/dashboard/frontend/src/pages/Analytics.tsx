...
export default function Analytics() {
  ...
  const hasQualityData = qualityChartData.length > 0 && qualityChartData.some(d => (d.count as number) > 0)
  const hasCtaData = ctaChartData.length > 0 && ctaChartData.some(d => (d.count as number) > 0)
  const hasIndustryData = industryData && industryData.length > 0 && industryData.some(d => d.count > 0)
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-surface-400 mt-1">
            {isTemplate ? 'Preview analytics with sample data' : 'Deep insights into your creative ads pipeline'}
          </p>
        </div>
        <Select
          options={[
